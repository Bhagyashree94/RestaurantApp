...
import UIKit
public typealias ColorVar = (hex: String, int: Int, alpha: CGFloat)

extension UIColor {
    
    // PRIMARY COLOR CODES:
    static let mBlack = UIColor.init(hex: "#000000")
    static let mWhite = UIColor.init(hex: "#ffffff")
    static let mGray = UIColor.init(hex: "#f5f5f5")
    static let mMearsBlue = UIColor.init(hex: "3B96ED")
    
    // SECONDARY COLOR CODES:
    static let mGray_E5 = UIColor.init(hex: "#e5e5e5")
    static let mGray_CC = UIColor.init(hex: "#cccccc")
    static let mGray_99 = UIColor.init(hex: "#999999")
    static let mGray_66 = UIColor.init(hex: "#666666")
    
    // TERTIARY COLOR CODES:
    static let mBlue = UIColor.init(hex: "#0099cc")
    static let mRed = UIColor.init(hex: "#df0000")
    static let mYellow = UIColor.init(hex: "#f4d14d")
    
    static let mLightBlue = UIColor.init(hex: "CBD2D3")
    static let mLightBlack = UIColor.init(hex: "#222222")
    
    // REWARDS TIERS COLOR CODES:
    static let mBronze = UIColor.init(hex: "#DCA175")
    static let mSilver = UIColor.init(hex: "#DCDCDC")
    static let mGold = UIColor.init(hex: "#FBD039")
    static let mPlatinum = UIColor.init(hex: "#9195A1")
    static let mDiamond = UIColor.init(hex: "#7AB8D2")
}
...
public extension UIColor {
    ...
    convenience init(hex: String?) {
        ...
        var cUnsignedInt: CUnsignedInt = 0
        Scanner(string: normalizedHexString).scanHexInt32(&cUnsignedInt)
        self.init(red: UIColorMasks.redValue(cUnsignedInt),
                  green: UIColorMasks.greenValue(cUnsignedInt),
                  blue: UIColorMasks.blueValue(cUnsignedInt),
                  alpha: UIColorMasks.alphaValue(cUnsignedInt))
    }
    ...
    func hexDescription(_ includeAlpha: Bool = false) -> String {
        ...
        let components = self.cgColor.components!.map { Int($0 * CGFloat(255)) }
        let color = String.init(format: "%02x%02x%02x", components[0], components[1], components[2])
        if includeAlpha {
            let alpha = String.init(format: "%02x", components[3])
            return "\(color)\(alpha)"
        }
        return color
    }
